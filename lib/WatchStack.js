...
const yargs = require('yargs')
    ...
    .default('region', 'us-east-1'), argv = yargs.argv;
if (argv.H) {
    yargs.showHelp();
    process.exit(0);
}
...
class WatchStack {
    ...
    initialize() {
        ...
        this.table.rows.on('select', (arg1, index, arg3) => {
            // self.screen.destroy();
            if (self.modal) {
                self.modal.destroy();
            }
    ...
    sortStackEventsByTimeDesc(s1, s2) {
        if (s1.Timestamp < s2.Timestamp) {
            return 1;
        }
        if (s1.Timestamp > s2.Timestamp) {
